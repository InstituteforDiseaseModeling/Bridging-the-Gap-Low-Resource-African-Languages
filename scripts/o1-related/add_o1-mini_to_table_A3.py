...
MODEL_PRETTY_NAME_TO_PLACE_ABOVE = 'GPT-4o'  # Model to place o1 results above using name in Table A3

# Load data
benchmarks = ['winogrande', 'mmlu-college_medicine', 'mmlu-clinical_knowledge', 'mmlu-virology', 'belebele']
table_a3_path = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__), '../../results/tables/table_A3.csv'))
table_a3 = pd.read_csv(table_a3_path, encoding='utf-8')
additional_dfs = {}
...
for label, row_df in additional_dfs.items():
    ...
    index_to_insert = table_a3.index[table_a3["Model (Evaluation Benchmark)"].str.contains(f"{MODEL_PRETTY_NAME_TO_PLACE_ABOVE} \\({label}\\)")][0]

    # Insert the row above the matched index
    table_a3 = pd.concat([table_a3.iloc[:index_to_insert], row_df, table_a3.iloc[index_to_insert:]]).reset_index(drop=True)

# Save to updated table
output_path = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__), '../../results/o1-related/table_A3_with_o1-mini.csv'))
table_a3.to_csv(output_path, index=False, encoding='utf-8')
